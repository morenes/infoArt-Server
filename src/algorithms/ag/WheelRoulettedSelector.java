 ...
 */
public class WheelRoulettedSelector implements INaturalSelector {
    ...
    /**
     * Metodo que selecciona nc crosomomas para operar con los operadores de cruce y mutacion
     * segun la probabilidad del metodo de la ruleta
    ...
    public synchronized void select(int a_howManyToSelect, Population a_from_pop,
        ...
        for(int i=0;i<a_from_pop.size();i++){
            sumFitness = sumFitness + Math.pow((1/a_from_pop.getChromosome(i).getFitnessValue()),2);
        }
        
        for(int i=0;i<a_from_pop.size();i++){
            cumVector[i] = Math.pow((1/a_from_pop.getChromosome(i).getFitnessValue()),2)/sumFitness;            
        }
