...
/**
 * Algoritmo Genetico con funcion de pertenencia Trapezoidal
 * @author David Gil Galvan
 */
public class GAFuzzyTrapezoidal extends GAFuzzy {
    
    /**
     * Nombre de la clase
     */
    protected String className="GAFuzzyTrapezoidal";
    
    /**
     *
     * Constructor del algoritmo genetico trapezoidal
     ...
     */
    public GAFuzzyTrapezoidal(int L, int T, double alfa1, double alfa2, int nc, double probCrossover, double[][] inputData, double[] outputData) {
        this.L=L;
        this.T=T;
        this.alfa1=alfa1;
        this.alfa2=alfa2;
    ...
    /**
     * Comprueba que el TSKModel es del tipo Trapezoidal
    ...
    protected void checkTSKModel() throws GAException{
        if (!(tskModel instanceof TrapezoidalTSKModel))
            throw new GAException(className+".checkTSKModel: El modelo TSK es incorrecto");
    }
    
    /**
     * Crea la funcion fitness segun el tsk Model trapezoidal
    ...
    protected FuzzyFitnessFunction createFuzzyFitnessFunction(double[][] inputData,double[] outputData,TSKModel tskModel) {
        return new TrapezoidalFuzzyFitnessFunction(inputData, outputData, tskModel);
    }
    
    /**
     * Metodo que calcula las restricciones espaciales a partir del cromosoma inicial y en funcion del tskModel con el
    ...
    protected void calculateV(Chromosome chromosome) {
        ...
        for (int i=0;i<numRules;i++) {  // Para cada regla
            for (int j=0;j<numAttributes;j++) { // Para cada abcd de la regla
                for (int k=0;k<numGenesForMF;k++) { // Para a o b o c o d
        ...
        try {
            ...
            for (int i=0;i<numRules;i++) {  // Para cada regla
                for (int j=0;j<numAttributes;j++) {
                    ...
                    c=((DoubleGene)genes[((i*numAttributes*numGenesForMF)+(j*numGenesForMF)+2)]).doubleValue();
                    double d=((DoubleGene)genes[((i*numAttributes*numGenesForMF)+(j*numGenesForMF)+3)]).doubleValue();
                    if (c>d) {
                        genes[((i*numAttributes*numGenesForMF)+(j*numGenesForMF)+2)].setAllele(new Double(d));
                        genes[((i*numAttributes*numGenesForMF)+(j*numGenesForMF)+3)].setAllele(new Double(c));
                    }
                }
            }
                    ...
                    while ((j<numAttributes)&&(!noChanged)) {
                        ...
                        for (int k=0;k<numRules;k++) {  // Para cada regla
                            if (i!=k) {
                                ...
                                dn=((DoubleGene)genes[((k*numAttributes*numGenesForMF)+(j*numGenesForMF)+3)]).doubleValue();
                        ...
                        if (best!=-1) {
                            ...
                            if(an>d){
                                genes[((i*numAttributes*numGenesForMF)+(j*numGenesForMF)+3)].setAllele(new Double(an));
                                genes[((best*numAttributes*numGenesForMF)+(j*numGenesForMF)+0)].setAllele(new Double(d));
                                noChanged=true;
                            }
                        }
                        j++;
    ...
    protected Chromosome createInitialChromosome() {
        ...
        double[][] param_d=tskModel.getParam_c();
        ...
        for (int i=0;i<numRules;i++) {
            for (int j=0;j<numAttributes;j++) {
                for (int k=0;k<numGenesForMF;k++) {
                    ...
                    if (k==3) { // d
                        (genes[((i*numAttributes*numGenesForMF)+(j*numGenesForMF)+k)]=new DoubleGene(-100,100)).setAllele(param_d[i][j]);
                    }
                }
            }
